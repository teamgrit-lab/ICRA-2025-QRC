...
class TwistHandler : public cobiz_bridge::MessageHandlerBase, 
                            public std::enable_shared_from_this<TwistHandler> {
public:
  TwistHandler() = default;
  ~TwistHandler() override = default;
...
private:
  ...
  void on_read_timeout(beast::error_code ec) {
    if (ec) {
      RCLCPP_ERROR(rclcpp::get_logger("TwistHandler"), "읽기 타임아웃 오류: %s", ec.message().c_str());
      return;
    }
  ...
  void on_read(beast::error_code ec, std::size_t bytes_transferred) {
    if (ec) {
      RCLCPP_ERROR(rclcpp::get_logger("TwistHandler"), "읽기 오류: %s", ec.message().c_str());
      return;
    }
    // 메시지 처리
    ...
    try {
      ...
      if (publisher_) {
        ...
        // RCLCPP_INFO(rclcpp::get_logger("TwistHandler"), "Twist 메시지 퍼블리시 완료");
      } else {
        RCLCPP_WARN(rclcpp::get_logger("TwistHandler"), "퍼블리셔가 없습니다");
      }

    }
    catch (const std::exception& e) {
      RCLCPP_ERROR(rclcpp::get_logger("TwistHandler"), "Twist 메시지 deserialize 실패: %s", e.what());
    }
    async_receive_message();
  }
  ...
  void on_write_timeout(beast::error_code ec) {
    if (ec) {
      // RCLCPP_ERROR(rclcpp::get_logger("TwistHandler"), "쓰기 타임아웃 오류: %s", ec.message().c_str());
      return;
    }
  ...
  void on_write(beast::error_code ec, std::size_t bytes_transferred) {
    if (ec) {
      RCLCPP_ERROR(rclcpp::get_logger("TwistHandler"), "쓰기 오류: %s", ec.message().c_str());
      return;
    }
    // 메시지 전송 완료
    async_send_message();
...
// 플러그인 클래스 등록
PLUGINLIB_EXPORT_CLASS(TwistHandler, cobiz_bridge::MessageHandlerBase)
