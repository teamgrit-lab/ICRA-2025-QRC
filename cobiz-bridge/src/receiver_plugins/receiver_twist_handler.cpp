...
class ReceiverTwistHandler : public cobiz_bridge::MessageHandlerBase,
                           public std::enable_shared_from_this<ReceiverTwistHandler> {
public:
  ReceiverTwistHandler() = default;
  ~ReceiverTwistHandler() override = default;
    ...
    try {
      ...
      ws_ptr->async_write(boost::asio::buffer(message),
        [this](beast::error_code ec, std::size_t bytes_transferred) {
          if (ec) {
            RCLCPP_ERROR(rclcpp::get_logger("ReceiverTwistHandler"), "쓰기 오류: %s", ec.message().c_str());
          }
        });
    }
    catch (const std::exception& e) {
      RCLCPP_ERROR(rclcpp::get_logger("ReceiverTwistHandler"), "Twist 메시지 처리 실패: %s", e.what());
    }
  }
...
private:
  ...
  void on_read_timeout(beast::error_code ec) {
    if (ec) {
      RCLCPP_ERROR(rclcpp::get_logger("ReceiverTwistHandler"), "읽기 타임아웃 오류: %s", ec.message().c_str());
      return;
    }
  ...
  void on_read(beast::error_code ec, std::size_t bytes_transferred) {
    if (ec) {
      if (ec != beast::websocket::error::closed)
        RCLCPP_ERROR(rclcpp::get_logger("ReceiverTwistHandler"), "읽기 오류: %s", ec.message().c_str());
      return;
    }
    ...
    //   } else {
    //     RCLCPP_WARN(rclcpp::get_logger("ReceiverTwistHandler"), "퍼블리셔가 없습니다");
    //   }
    // }
    // catch (const std::exception& e) {
    //   RCLCPP_ERROR(rclcpp::get_logger("ReceiverTwistHandler"), "JSON 파싱 실패: %s", e.what());
    // }
  ...
  void on_write_timeout(beast::error_code ec) {
    ...
    ws_ptr->async_write(boost::asio::buffer(message),
      [this](beast::error_code ec, std::size_t) {
        if (ec) {
          RCLCPP_ERROR(rclcpp::get_logger("ReceiverTwistHandler"), "핑 메시지 전송 실패: %s", ec.message().c_str());
        }
        async_send_message();
      });
  }
...
// 플러그인 클래스 등록
PLUGINLIB_EXPORT_CLASS(ReceiverTwistHandler, cobiz_bridge::MessageHandlerBase)
