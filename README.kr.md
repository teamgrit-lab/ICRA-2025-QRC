...
5. [Sensor 메세지 전송 규칙](#Sensor-메세지-전송-규칙)
6. [빌드 및 실행](#빌드-및-실행)
...
센서 데이터를 UI에 정확히 표시하기 위해, 메시지는 아래와 같은 형식으로 전송해야 합니다.
...
모든 센서 데이터는 data 객체 안에 위치해야 합니다.

각 센서 항목은 {센서 이름: 값} 형태로 구성되어야 합니다.

battery 항목은 별도로 UI의 배터리 표시 영역에 사용되며, 반드시 float 값으로 전송되어야 합니다. (battery 이외의 값들은 별도의 UI에 글자로 표시됩니다.)

UI 시스템은 {센서 이름: 값} 구조를 기반으로 센서 이름과 값을 1:1 매핑하여 렌더링합니다. 뎁스가 깊어질 경우, 이 매핑이 깨지면서 UI 표현에 문제가 발생하게 됩니다.

때문에 아래의 예시처럼 data 안에 중첩된 객체를 허용하지 않습니다. (2단 뎁스를 초과하여 사용 할 수 없습니다.)
